...
import 'package:Wanderer/widgets/Organization/Events/EventItemWidget.dart';
...
class _EventListeState extends State<EventList> {
  ...
  Future _fetchListSort(int p, int filterKey) async {
    var rep = await EventService.getListSort(p, filterKey, _locationData)
  ...
  Future _fetchListSearch(int p, int filterKey, String searchKey) async {
    var rep =
        await EventService.getListSearch(p, filterKey, searchKey, _locationData)
            as Iterable<OrganizedEvent>;
    setState(() {
      litems = litems..addAll(rep);
      page = p + 1;
